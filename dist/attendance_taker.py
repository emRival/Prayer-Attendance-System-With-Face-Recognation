...
class FaceRecognizer:
    ...
    def load_known_faces(self):
        if os.path.exists("data/features_all.csv"):
            ...
            for i in range(csv_rd.shape[0]):
                self.face_name_known_list.append((csv_rd.iloc[i][0], csv_rd.iloc[i][1]))  # (name, class)
                self.face_features_known_list.append([float(x) if x else 0.0 for x in csv_rd.iloc[i][2:130]])
    ...
    def get_session(self, current_time):
        ...
        for segment in self.prayer_times:
            ...
            # Cek rentang waktu termasuk yang melewati tengah malam
            if start <= end:
    ...
    def record_attendance(self, name, class_name, session, timestamp):
        record = {"name": name, "class": class_name, "session": session, "date": timestamp.split(' ')[0], "time": timestamp.split(' ')[1]}
        
        # Prevent duplicate entries: Check if the same person, session, and date already exists
        if not any(existing_record['name'] == record['name'] and existing_record['class'] == record['class'] and
                   existing_record['session'] == record['session'] and existing_record['date'] == record['date']
                   for existing_record in self.absent_records):
...
if __name__ == "__main__":
    # Baca waktu shalat dari config/time_segment.json
